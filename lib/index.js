...
export default async function RiN(srcDir, files = "all", appView, options) {
    ...
    appView = await (async () => {
        if (appView && "default" != appView && await access(path.resolve(srcDir, appView), fs.constants.R_OK).catch(() => false))
            return path.resolve(srcDir, appView);
        if (!await access(path.resolve(srcDir, "views/App.html"), fs.constants.R_OK).catch(() => true))
            return path.resolve(srcDir, "views/App.html");
        if (!await access(path.resolve(srcDir, "App.html"), fs.constants.R_OK).catch(() => true))
            return path.resolve(srcDir, "App.html");
        throw new Error(ERRORS.MAIN_VIEW_NOT_FOUND);
    })();
    const compiler = new RiNCompiler(srcDir, "default", options);
